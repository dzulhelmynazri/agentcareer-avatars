...
import type { Point, Silhouette } from "./shape";
import { STATE_BY_ID } from "./states";
import type { Pose, StateDef, StateId } from "./states";
...
const NO_LOOK: Look = { mix: 0, pitch: 0, spin: 0, wander: 1, yaw: 0 };
...
export class BotEngine {
  ...
  private posed(
    ...
    if (def.baseFace && expr) {
      pose = { ...pose, eyes: expr.eyes, gaze: expr.gaze, split: expr.split };
    }
    return pose;
  }
  ...
  setState(id: StateId, now: number) {
    ...
    const { morph } = STATE_BY_ID.get(this.cur)!;
  ...
  sample(now: number): BotFrame {
    ...
    const notif = pose.notif ? { r: pose.notif.r * R, x: nx, y: ny } : null;
    const notch = pose.notif ? { r: pose.notif.notch * R, x: nx, y: ny } : null;
